...
function parseStreamChunk(event: string): OpenRouterStreamChunk | null {
  ...
  try {
    return JSON.parse(payload) as OpenRouterStreamChunk;
  } catch {
    return null;
  }
}
...
export async function sendMessage(
  ...
  try {
    ...
    while (true) {
      ...
      const endsWithDelimiter = /\r?\n\r?\n$/.test(pending);
      pending = endsWithDelimiter ? '' : (events.pop() || pending);
      ...
      for (const event of events) {
        ...
        if (chunk.error?.message) {
          throw new ApiError(chunk.error.message, 'STREAM_ERROR', response.status);
        }

        const content = chunk.choices?.[0]?.delta?.content;
        if (content) {
          assistantContent += content;
          options?.onChunk?.(assistantContent);
        }
